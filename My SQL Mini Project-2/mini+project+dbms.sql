...
select AB.STADIUM_ID,AB.STADIUM_NAME,AB.CITY, COUNT(*) AS NO_OF_MATCHES_CONDUCTED
FROM ipl_stadium AB
join ipl_match_schedule BC
ON AB.STADIUM_ID = BC.STADIUM_ID
GROUP BY AB.STADIUM_NAME
...
(select count(*) from ipl_match m join ipl_match_schedule ms 
on m.match_id = ms.match_id
where ms.stadium_id = s.stadium_id and (toss_winner = match_winner)) /
(select count(*) from ipl_match_schedule ms 
where ms.stadium_id = s.stadium_id) * 100 as 'Percentage of Wins by teams who won the toss (%)'
from ipl_stadium s;
...
SELECT AB.BIDDER_NAME,AB.BIDDER_ID,CD.BID_TEAM,BC.NO_OF_BIDS 
FROM ipl_Bidder_details AB
JOIN ipl_bidder_points BC
on AB.BIDDER_ID = BC.BIDDER_ID
JOIN ipl_bidding_details CD
on CD.BIDDER_ID = BC.BIDDER_ID
...
select AB.TEAM_ID,BC.WIN_DETAILS,AB.TEAM_NAME
FROM IPL_TEAM AS AB
CROSS JOIN IPL_MATCH AS BC
WHERE AB.TEAM_ID = BC.MATCH_WINNER



#6.	Display total matches played, total matches won and total matches lost by team along with its team name.

select BC.TEAM_NAME,AB.MATCHES_PLAYED,AB.MATCHES_WON, AB.MATCHES_LOST
FROM IPL_TEAM BC
JOIN IPL_TEAM_STANDINGS AB
ON BC.TEAM_ID=AB.TEAM_ID
...
select AB.PLAYER_ROLE,AB.REMARKS,BC.PLAYER_ID,BC.PLAYER_NAME
from ipl_team_players AB
join ipl_Player BC
on AB.PLAYER_ID = BC.PLAYER_ID   
WHERE AB.PLAYER_ROLE = 'Bowler' AND AB.REMARKS='TEAM - MI'

#8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

SELECT AB.PLAYER_ROLE,AB.REMARKS,BC.TEAM_ID,BC.TEAM_NAME,COUNT(*) AS COUNT1
from ipl_team_players AB
join ipl_team BC
on AB.TEAM_ID = BC.TEAM_ID  
WHERE AB.PLAYER_ROLE='All-Rounder'
GROUP BY  BC.TEAM_NAME
HAVING COUNT1 > 4
